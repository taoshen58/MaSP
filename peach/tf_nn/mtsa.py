...
def stacking_mtsa(
    ...
    """
    with tf.variable_scope(scope or 'stacking_mtsa'):
        ...
        for layer_idx in range(layer_num):
            with tf.variable_scope('layer_%d' % layer_idx):
                ...
                y = multi_mask_tensorized_self_attn(
                    ...
                    output_hn=hn, accelerate=True, merge_var=False, scope='mtsa'
                )
...
def multi_mask_tensorized_self_attn(
    ...
    otherwise out_hn = "hn"
    """
    with tf.variable_scope(scope or 'multi_mask_tensorized_self_attn'):
    ...
    with tf.name_scope('mask_generation'):
        ...
        if use_direction:
            ...
            if not attn_self:
                ...
                direct_mask = tf.cast(tf.ones([1, 1, 1], dtype=tf.int32), tf.bool)
...
def bn_dense_layer(
    ...
    assert len(input_tensor.get_shape().as_list()) == 3 or len(input_tensor.get_shape().as_list()) == 2
    is_seq_data = (len(input_tensor.get_shape().as_list()) == 3)
    ...
    with tf.variable_scope(scope or 'bn_dense_layer'):
        ...
        if is_seq_data:
            ...
            input_tensor_rsp = dropout(input_tensor, keep_prob, is_train)
...
def multi_dim_souce2token_self_attn_lagacy(rep_tensor, rep_mask, scope=None,
...
def multi_dim_souce2token_self_attn(rep_tensor, rep_mask, scope=None,
                                    keep_prob=1., is_train=None, wd=0., activation='elu',
                                    tensor_dict=None, name=None, multi_head=None):
    bs, sl = tf.shape(rep_tensor)[0], tf.shape(rep_tensor)[1]

    if isinstance(multi_head, int):
        rep_tensor_attn = split_head(rep_tensor, multi_head)  # head,bs,sl,dim
        ivec = rep_tensor_attn.get_shape().as_list()[-1]
        rep_tensor_attn = tf.reshape(rep_tensor_attn, [multi_head * bs, sl, ivec]) # head*bs,sl,dim

        # mask
        rep_mask = tf.tile(tf.expand_dims(rep_mask, 0), [multi_head, 1, 1])
        rep_mask = tf.reshape(rep_mask, [multi_head * bs, sl])
    else:
        rep_tensor_attn = rep_tensor  # bs,sl,dim
        ivec = rep_tensor_attn.get_shape().as_list()[-1]

    with tf.variable_scope(scope or 'multi_dimensional_attention'):
        map1 = bn_dense_layer(rep_tensor_attn, ivec, True, 0., 'bn_dense_map1', activation,
                              False, wd, keep_prob, is_train, 1, False)
        map2 = bn_dense_layer(map1, ivec, True, 0., 'bn_dense_map2', 'linear',
                              False, wd, keep_prob, is_train, 1, False)
        map2_masked = exp_mask_v2(map2, rep_mask, multi_head=False, high_dim=True)

        soft = tf.nn.softmax(map2_masked, -2)  # [head,]bs,sl,vec
        attn_output = tf.reduce_sum(soft * rep_tensor_attn, -2)  # [head]bs,vec
    ...
    if isinstance(multi_head, int):
        attn_output = tf.reshape(attn_output, [multi_head, bs, ivec])
        res_return = tf.reshape(tf.transpose(attn_output, [1, 0, 2]), [bs, multi_head * ivec])
    else:
        res_return = attn_output
